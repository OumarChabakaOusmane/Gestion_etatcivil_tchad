        ...
        const content = `
            <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 600px; margin: 0 auto; background: linear-gradient(135deg, #f8f9fa 0%, #e9ecef 100%); border-radius: 16px; overflow: hidden; box-shadow: 0 10px 30px rgba(0, 32, 91, 0.1);">
                ...
                <div style="padding: 40px 30px; background: white;">
                    <h2 style="color: #00205b; margin: 0 0 20px 0; font-size: 20px;">Vérification de votre compte</h2>
                    <p style="color: #495057; font-size: 16px; line-height: 1.6;">
                        Bonjour <strong style="color: #00205b;">${userName}</strong>,<br>
                        Voici votre code de vérification :
                    </p>
                    <div style="background: #f8f9fa; border: 2px dashed #00205b; border-radius: 12px; padding: 25px; text-align: center; margin: 30px 0;">
                        <div style="font-size: 36px; font-weight: 700; letter-spacing: 8px; color: #00205b; font-family: 'Courier New', monospace;">
                            ${otpCode}
                        </div>
                        <div style="margin-top: 15px; font-size: 14px; color: #6c757d;">⏰ Valide pendant 10 minutes</div>
                    </div>
                    <div style="background: #fff3cd; border-left: 4px solid #ffc107; padding: 15px 20px; border-radius: 0 8px 8px 0;">
                        <strong style="color: #856404;">Important :</strong>
                        <ul style="margin: 8px 0 0 0; padding-left: 20px; color: #856404;">
                            <li>Ne partagez jamais ce code avec personne</li>
                            <li>Vérifiez votre dossier <strong>SPAM</strong> si vous ne voyez pas l'email</li>
                            <li>Le code expire automatiquement après 10 minutes</li>
                        </ul>
                    </div>
                </div>
                <div style="background: #f8f9fa; padding: 20px; text-align: center; border-top: 1px solid #e9ecef;">
                    <p style="margin: 0; font-size: 12px; color: #adb5bd;">© 2026 SIGEC-TCHAD - Ne pas répondre à cet email.</p>
                </div>
            </div>
        `;
        const text = `SIGEC-TCHAD - Votre code de vérification est : ${otpCode}. Valide 10 minutes.`;
    ...
    async sendNotificationValidation(userEmail, userName, typeActe, idDemande) {
        ...
        const content = `
            <div style="padding: 20px; font-family: sans-serif;">
                ...
                Votre demande de <strong>${typeActe}</strong> (Réf: <strong>${idDemande.slice(-8).toUpperCase()}</strong>) a été validée.</p>
                <div style="text-align: center; margin: 20px 0;">
                    <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/mes-demandes"
                       style="background: #28a745; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: 600;">
                        📥 Accéder à mes documents
                    </a>
                </div>
            </div>
        `;
        const text = `Votre demande de ${typeActe} (Réf: ${idDemande.slice(-8).toUpperCase()}) a été approuvée.`;
    ...
    async sendNotificationRejet(userEmail, userName, typeActe, motif) {
        ...
        const content = `
            <div style="padding: 20px; font-family: sans-serif;">
                ...
                <p>Cher(e) <strong>${userName}</strong>,<br>
                Votre demande de <strong>${typeActe}</strong> a été rejetée.</p>
                <div style="background: #fff3cd; border: 2px solid #ffc107; border-radius: 8px; padding: 20px; margin: 20px 0;">
                    <strong>Motif :</strong> ${motif}
                </div>
                <div style="text-align: center; margin: 20px 0;">
                    <a href="${process.env.FRONTEND_URL || 'http://localhost:5173'}/mes-demandes"
                       style="background: #ffc107; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: 600;">
                        🔄 Consulter ma demande
                    </a>
                </div>
            </div>
        `;
        const text = `Votre demande de ${typeActe} a été rejetée. Motif: ${motif}.`;
        return this.sendEmail(userEmail, subject, content, text);
    }
    ...
    async sendPasswordResetEmail(userEmail, userName, resetUrl) {
        ...
        const content = `
            <div style="padding: 20px; font-family: sans-serif;">
                ...
                <p>Cliquez sur le bouton ci-dessous pour réinitialiser votre mot de passe (lien valide 1 heure) :</p>
                <div style="text-align: center; margin: 20px 0;">
                    <a href="${resetUrl}"
                       style="background: #00205b; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: 600;">
                        🔑 Réinitialiser mon mot de passe
                    </a>
                </div>
                <p style="font-size: 12px; color: #6c757d;">Si vous n'avez pas demandé cette réinitialisation, ignorez cet email.</p>
            </div>
        `;
        const text = `Bonjour ${userName}, réinitialisez votre mot de passe : ${resetUrl}`;
        return this.sendEmail(userEmail, subject, content, text);
    }
